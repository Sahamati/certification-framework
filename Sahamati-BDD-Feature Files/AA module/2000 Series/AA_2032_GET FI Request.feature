Feature: On calling GET FI fetch Request API, verify that on sending FI fetch request when consent is revoked, the
  error response is received.

  Scenario: 2032_1 GET FI fetch Request API, use the pre-linked user details from settings, and follow below steps:
  ...
  13. Ask the user to revoke the consent and confirm.
  ...
  Scenario: 2032_2 GET FI fetch Request API, use the pre-linked user details from settings, and follow below steps:
  1. Send a valid consent request.
  2. Set the Post consent response in FIP mock server.
  3. Set the consent notification response in FIU mock server.
  4. Ask the user to approve the consent and confirm.
