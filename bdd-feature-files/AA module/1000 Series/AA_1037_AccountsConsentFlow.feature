Feature: On calling FIP and FIU Notifications request API, verify that on user changing the consent status, the
  status change is notified to FIU.

  Scenario: 1037_1 On calling FIP and FIU Notifications request API, use the pre-linked user details from settings.
  ...
  Scenario: 1037_2 On calling FIP and FIU Notifications request API, use the pre-linked user details from settings.
  and set the POST consent response on regular and alternate mock server. Send a valid consent request.
  Ask the user to approve the consent with one account from regular and alternative mock FIP each and confirm.
  Send a valid consent handle and send a consent GET request. Get the POST consent request received on the mock
  FIP application. Set the consent notification response on regular and alternate mock server. Ask the user to
  ...
  Scenario: 1037_3 On calling FIP and FIU Notifications request API, use the pre-linked user details from settings.
  and set the POST consent response on regular and alternate mock server. Send a valid consent request.
  Ask the user to approve the consent with one account from regular and alternative mock FIP each and confirm.
  Send a valid consent handle and send a consent GET request. Get the POST consent request received on the mock
  FIP application. Set the consent notification response on regular and alternate mock server. Ask the user to
